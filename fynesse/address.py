...
def predict_price(latitude, longitude, date, property_type):
    ...
    for i,r in dataset.iterrows():
        ...
        x.append([1] \
               ...
               + (datetime.datetime.strptime(t_date, '%y/%m/%d') - datetime.date(1995, 1, 1)).total_seconds() // (30*24*60*60))
    ...
    features = np.array([[1] \
               ...
               + (datetime.datetime.strptime(date, '%y/%m/%d')  - datetime.date(1995, 1, 1)).total_seconds() // (30*24*60*60)])
